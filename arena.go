package llrb

import "unsafe"
import "sort"

// MEMUtilization expected.
const MEMUtilization = 0.95

const sizeinterval = 32

type Arena struct {
	capacity   int           // memory managed by this arena
	minblock   int           // minimum block size allocatable by arena
	maxblock   int           // maximum block size allocatable by arena
	numblocks  int           // number of blocks in a pool
	blocksizes []int         // sorted list of block-sizes managed by this arena
	pools      map[int]Pools // size -> sorted list of Pool
}

func NewArena(capacity, minblock, maxblock, numblocks int) *Arena {
	arena := &Arena{
		capacity:  capacity,
		minblock:  minblock,
		maxblock:  maxblock,
		numblocks: numblocks,
		pools:     make(map[int]Pools),
	}
	arena.blocksizes = Blocksizes(arena.minblock, arena.maxblock)
	for _, size := range arena.blocksizes {
		arena.pools[size] = make(Pools, 0)
	}
	return arena
}

func (arena *Arena) Alloc(n int) (ptr unsafe.Pointer, pool *Pool) {
	var ok bool

	if largest := arena.blocksizes[len(arena.blocksizes)-1]; n > largest {
		return nil, nil
	}
	size := SuitableSize(arena.blocksizes, n)
	for _, pool = range arena.pools[size] {
		if ptr, ok = pool.Alloc(); ok {
			return ptr, pool
		}
	}
	pool = NewPool(size, arena.numblocks)
	arena.pools[size] = append(arena.pools[size], pool)
	sort.Sort(arena.pools[size])
	ptr, _ = pool.Alloc()
	return ptr, pool
}

func (arena *Arena) Close() {
	for _, pools := range arena.pools {
		for _, pool := range pools {
			pool.Close()
		}
	}
}

func SuitableSize(sizes []int, minsize int) int {
	switch len(sizes) {
	case 1:
		return sizes[0]

	case 2:
		if minsize < sizes[0] {
			return sizes[0]
		}
		return sizes[1]

	default:
		pivot := len(sizes) / 2
		if sizes[pivot] < minsize {
			return SuitableSize(sizes[pivot+1:], minsize)
		}
		return SuitableSize(sizes[:pivot+1], minsize)
	}
}

func Blocksizes(minblock, maxblock int) []int {
	if maxblock < minblock { // validate and cure the input params
		panic("minblock < maxblock")
	} else if (maxblock % sizeinterval) != 0 {
		maxblock = maxblock + (sizeinterval - (maxblock % sizeinterval))
	}

	nextsize := func(from int) int {
		size := from + 32
		for (float64(from+size)/2.0)/float64(size) > MEMUtilization {
			size += 32
		}
		return size
	}

	sizes := []int{}
	for size := minblock; size <= maxblock; {
		sizes = append(sizes, size)
		size = nextsize(size)
	}
	return sizes
}
